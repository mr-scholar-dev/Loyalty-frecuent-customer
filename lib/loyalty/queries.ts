...
export type RecoverResult =
  | { ok: true; token: string }
  | { ok: false; reason: "not_found" | "blocked" | "error" };

/**
 * Card recovery (§13): match a membership by organization + phone + plate and
 * REISSUE its public token (the plaintext is never stored, so we mint a fresh
 * one and revoke the old). Returns the new plaintext token to show the card.
 *
 * SECURITY: phone+plate is a weak secret. Production must add rate limiting and
 * ideally an OTP step; this is the MVP "sin proveedor de mensajería" path.
 */
export async function recoverMembership(input: {
  slug: string;
  phoneNormalized: string;
  plateNormalized: string;
  ...
  // The phone must belong to the same customer as the plate.
  ...
  if (membership.status !== "active") return { ok: false, reason: "blocked" };
  ...
  await admin.from("audit_logs").insert({
    organization_id: org.id,
    action: "membership.recover",
    entity_type: "membership",
    entity_id: membership.id,
    after_data: { note: "Tarjeta recuperada (token reemitido)" },
  });
